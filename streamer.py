...
class MultiCameraStreamer:
    ...
    def start(self, source_id):
        ...
        # Якщо вже запущено, просто повертаємо True
        if source_id in self.running and self.running[source_id]:
            return True
        ...
        try:
            ...
            if cap.isOpened():
                ...
                logger.error(f"Could not open source {source_id}")
        except Exception as e:
            logger.error(f"Error starting {source_id}: {e}")
        return False
    ...
    def _write_to_record(self, source_id, frame):
        if source_id not in self.recorders:
            ...
            h, w = frame.shape[:2]
            filename = f"recordings/cam_{source_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            self.recorders[source_id] = cv2.VideoWriter(filename, fourcc, 20.0, (w, h))
            logger.info(f"Started recording for {source_id}: {filename}")
    ...
    def stop_recording(self, source_id):
        ...
        if source_id in self.recorders:
            ...
            logger.info(f"Stopped recording for {source_id}")
